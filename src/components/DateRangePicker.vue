...
const { startDate: initialStartDate, endDate: initialEndDate } = defineProps(["startDate", "endDate"]);
...
<template>
    <div class="date-picker-container">
        <v-btn variant="outlined" append-icon="calendar_month" @click="toggleStartDate">
        ...
        <v-date-picker v-if="startDateToggled" class="start-date-picker" bg-color="white" color="blue-darken-2"
            hide-header elevation="5" :allowed-dates="allowedStartDate" v-model="startDate"
            @update:modelValue="updateStartDate" />

        <v-icon icon="arrow_right_alt" class="mx-2"></v-icon>

        <v-btn variant="outlined" append-icon="calendar_month" @click="toggleEndDate">{{ showDate(endDate) ?? "End date"
            }}</v-btn>
        <v-date-picker v-if="endDateToggled" class="end-date-picker" bg-color="white" color="blue-darken-2" hide-header
            elevation="5" :allowed-dates="allowedEndDate" v-model="endDate" @update:modelValue="updateEndDate" />
    </div>
</template>
